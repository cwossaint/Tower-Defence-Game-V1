...
class Glue(Tower):
    ...
    def fire_projectile(self):
        ...
        if self.target and self.attack_timer >= self.attack_delay:
            ...
            projectile = Glue_Projectile(self.x + (self.sprite.get_width() / 2), self.y + (self.sprite.get_height()  / 2), self.target_coords, self.damage, self.slow, self.splash_range, self.slow_duration)
            self.attack_timer = 0
